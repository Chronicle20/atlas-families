...
type Processor interface {
	AddJunior(seniorId uint32, juniorId uint32) model.Provider[FamilyMember]
	RemoveMember(characterId uint32, reason string) model.Provider[[]FamilyMember]
	BreakLink(characterId uint32, reason string) model.Provider[[]FamilyMember]
	AwardRep(characterId uint32, amount uint32, source string) model.Provider[FamilyMember]
	DeductRep(characterId uint32, amount uint32, reason string) model.Provider[FamilyMember]
	ResetDailyRep() model.Provider[int64]
	UpdateWorld(characterId uint32, world byte) model.Provider[FamilyMember]
	UpdateLevel(characterId uint32, level uint16) model.Provider[FamilyMember]
...
func NewProcessor(db *gorm.DB, log logrus.FieldLogger, administrator Administrator, producer EventProducer) Processor {
...
func (p *ProcessorImpl) AddJunior(seniorId uint32, juniorId uint32) model.Provider[FamilyMember] {
...
func (p *ProcessorImpl) RemoveMember(characterId uint32, reason string) model.Provider[[]FamilyMember] {
	return func() ([]FamilyMember, error) {
		p.log.WithFields(logrus.Fields{
...
func (p *ProcessorImpl) BreakLink(characterId uint32, reason string) model.Provider[[]FamilyMember] {
	return func() ([]FamilyMember, error) {
		p.log.WithFields(logrus.Fields{
			"characterId": characterId,
			"reason":      reason,
...
func (p *ProcessorImpl) AwardRep(characterId uint32, amount uint32, source string) model.Provider[FamilyMember] {
	return func() (FamilyMember, error) {
		p.log.WithFields(logrus.Fields{
			"characterId": characterId,
			"amount":      amount,
...
func (p *ProcessorImpl) DeductRep(characterId uint32, amount uint32, reason string) model.Provider[FamilyMember] {
	return func() (FamilyMember, error) {
		p.log.WithFields(logrus.Fields{
			"characterId": characterId,
			"amount":      amount,
...
func (p *ProcessorImpl) ResetDailyRep() model.Provider[int64] {
...
func (p *ProcessorImpl) UpdateWorld(characterId uint32, world byte) model.Provider[FamilyMember] {
	return func() (FamilyMember, error) {
		p.log.WithFields(logrus.Fields{
			"characterId": characterId,
			"world":       world,
...
func (p *ProcessorImpl) UpdateLevel(characterId uint32, level uint16) model.Provider[FamilyMember] {
	return func() (FamilyMember, error) {
		p.log.WithFields(logrus.Fields{
			"characterId": characterId,
			"level":       level,
...
func (p *ProcessorImpl) AddJuniorAndEmit(transactionId string, seniorId uint32, juniorId uint32) model.Provider[FamilyMember] {
	return func() (FamilyMember, error) {
		...
		if emitErr := p.producer.EmitLinkCreated(transactionId, seniorId, seniorId, juniorId); emitErr != nil {
			p.log.WithError(emitErr).Error("Failed to emit link created event")
		}
...
func (p *ProcessorImpl) RemoveMemberAndEmit(transactionId string, characterId uint32, reason string) model.Provider[[]FamilyMember] {
	return func() ([]FamilyMember, error) {
		...
		// Execute business logic
		result, err := p.RemoveMember(characterId, reason)()
		if err != nil {
			return []FamilyMember{}, err
		}
		...
		if memberModel.HasSenior() {
			if emitErr := p.producer.EmitLinkBroken(transactionId, characterId, *memberModel.SeniorId(), characterId, reason); emitErr != nil {
				p.log.WithError(emitErr).Error("Failed to emit link broken event for senior")
			}
		}
		
		for _, juniorId := range memberModel.JuniorIds() {
			if emitErr := p.producer.EmitLinkBroken(transactionId, characterId, characterId, juniorId, reason); emitErr != nil {
				p.log.WithError(emitErr).Error("Failed to emit link broken event for junior")
			}
		}
...
func (p *ProcessorImpl) BreakLinkAndEmit(transactionId string, characterId uint32, reason string) model.Provider[[]FamilyMember] {
	return func() ([]FamilyMember, error) {
		...
		// Execute business logic
		result, err := p.BreakLink(characterId, reason)()
		if err != nil {
			return []FamilyMember{}, err
		}
...
func (p *ProcessorImpl) AwardRepAndEmit(transactionId string, characterId uint32, amount uint32, source string) model.Provider[FamilyMember] {
	return func() (FamilyMember, error) {
		// Execute business logic
		result, err := p.AwardRep(characterId, amount, source)()
		if err != nil {
			// Emit error event
			if emitErr := p.producer.EmitRepError(transactionId, characterId, "AWARD_REP_FAILED", err.Error(), amount); emitErr != nil {
				p.log.WithError(emitErr).Error("Failed to emit rep error event")
			}
			return FamilyMember{}, err
		}
		...
		if emitErr := p.producer.EmitRepGained(transactionId, characterId, amount, result.DailyRep(), source); emitErr != nil {
			p.log.WithError(emitErr).Error("Failed to emit rep gained event")
		}
...
func (p *ProcessorImpl) DeductRepAndEmit(transactionId string, characterId uint32, amount uint32, reason string) model.Provider[FamilyMember] {
	return func() (FamilyMember, error) {
		// Execute business logic
		result, err := p.DeductRep(characterId, amount, reason)()
		if err != nil {
			// Emit error event
		...
		if emitErr := p.producer.EmitRepRedeemed(transactionId, characterId, amount, reason); emitErr != nil {
			p.log.WithError(emitErr).Error("Failed to emit rep redeemed event")
		}
